#!/usr/bin/env python3
"""
Santa Barbara Public Library scraper - Library of Things
Info: https://library.santabarbaraca.gov/books-more/library-things
...
BASE_URL = "https://catalog.sbplibrary.org"
...
def categorize_item(name, description=""):
    ...
    if any(word in text for word in ['cake', 'pan', 'kitchen', 'cook', 'bake', 'food', 'bundt']):
        return "Home & Kitchen"
    if any(word in text for word in ['tent', 'camping', 'hike', 'bike', 'sport', 'fitness', 'yoga', 'wellness', 'telescope', 'binocular', 'park pass', 'state park']):
...
def scrape_catalog():
    """Scrape items from Santa Barbara Library catalog"""
    print("Starting Santa Barbara Public Library scrape...")
    print("=" * 60)
    ...
    try:
        ...
        with open('santa_barbara_library_catalog.html', 'w', encoding='utf-8') as f:
            f.write(response.text)
        print("✓ Saved catalog page to santa_barbara_library_catalog.html\n")
        ...
        result_items = (
            soup.find_all('div', class_=re.compile(r'(result|item|record|title-cell)')) or
            soup.find_all('div', class_=re.compile(r'browse')) or
        ...
        for idx, result in enumerate(result_items[:100], 1):
            try:
                # Extract title
                title_elem = (
                    result.find('a', class_=re.compile(r'title')) or
                    result.find('span', class_=re.compile(r'title')) or
                    result.find('div', class_=re.compile(r'title')) or
                    result.find('a', href=re.compile(r'/Record/|/GroupedWork/'))
                )
                ...
                # Extract description
                description = ""
                desc_elem = result.find(['p', 'div', 'span'], class_=re.compile(r'(description|summary|note)'))
                if desc_elem:
...
def main():
    ...
    # Save to file
    output_file = "santa_barbara_library_data.json"
